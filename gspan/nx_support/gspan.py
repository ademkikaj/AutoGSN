...
class DFSedge(object):
    ...
    def __eq__(self, other):
        ...
        return (
            self.frm == other.frm and self.to == other.to and self.vevlb == other.vevlb
        )
...
class DFScode(list):
    ...
    def get_num_vertices(self):
        ...
        return len(
            set([dfsedge.frm for dfsedge in self] + [dfsedge.to for dfsedge in self])
        )
...
class gSpan(object):
    ...
    def __init__(
        ...
        if self._max_num_vertices < self._min_num_vertices:
            print(
                "Max number of vertices can not be smaller than "
                "min number of that.\n"
    ...
    def time_stats(self):
        ...
        for fn in func_names:
            time_deltas[fn] = round(
                self.timestamps[fn + "_out"] - self.timestamps[fn + "_in"], 2
            )

        print("Read:\t{} s".format(time_deltas["_read_graphs"]))
        print("Mine:\t{} s".format(time_deltas["run"] - time_deltas["_read_graphs"]))
    ...
    def _generate_1edge_frequent_subgraphs(self):
        ...
        for vlb, cnt in vlb_counter.items():
            if cnt >= self._min_support:
                g = Graph(gid=next(self._counter), is_undirected=self._is_undirected)
                g.add_vertex(0, vlb)
    ...
    def _report(self, projected):
        ...
        # pd.concat([self._report_df, self._report_df])
        ...
        self.nx_graphs.append(g.to_nx_graph())
        if self._visualize:
            g.plot()
        if self._where:
    ...
    def _get_backward_edge(self, g, e1, e2, history):
        ...
        for to, e in g.vertices[e2.to].edges.items():
            ...
            if self._is_undirected:
                if e1.elb < e.elb or (
                    e1.elb == e.elb and g.vertices[e1.to].vlb <= g.vertices[e2.to].vlb
                ):
                    return e
            else:
                if g.vertices[e1.frm].vlb < g.vertices[e2.to].vlb or (
                    g.vertices[e1.frm].vlb == g.vertices[e2.to].vlb and e1.elb <= e.elb
                ):
                    return e
    ...
    def _is_min(self):
        ...
        if len(self._DFScode) == 1:
            return True
        g = self._DFScode.to_graph(
            gid=VACANT_GRAPH_ID, is_undirected=self._is_undirected
        )
        ...
        def project_is_min(projected):
            ...
            for p in projected:
                history = History(g, p)
                edges = self._get_forward_pure_edges(
            ...
            for rmpath_i in rmpath:
                ...
                for p in projected:
                    history = History(g, p)
                    edges = self._get_forward_rmpath_edges(
    ...
    def _subgraph_mining(self, projected):
        ...
        for p in projected:
            ...
            for rmpath_i in rmpath[::-1]:
                e = self._get_backward_edge(
                    g, history.edges[rmpath_i], history.edges[rmpath[0]], history
                )
                if e is not None:
                    backward_root[(self._DFScode[rmpath_i].frm, e.elb)].append(
                        PDFS(g.gid, e, p)
                    )
            # pure forward
            ...
            for rmpath_i in rmpath:
                ...
                for e in edges:
                    forward_root[
                        (self._DFScode[rmpath_i].frm, e.elb, g.vertices[e.to].vlb)
                    ].append(PDFS(g.gid, e, p))

        # backward
        for to, elb in backward_root:
            self._DFScode.append(
                DFSedge(maxtoc, to, (VACANT_VERTEX_LABEL, elb, VACANT_VERTEX_LABEL))
            )
        ...
        for frm, elb, vlb2 in forward_root:
            self._DFScode.append(
                DFSedge(frm, maxtoc + 1, (VACANT_VERTEX_LABEL, elb, vlb2))
            )
